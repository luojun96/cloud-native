...
- [Go 是传值还是传引用？](#go-是传值还是传引用)
- [Golang 相对于其他语言有哪些特点](#golang-相对于其他语言有哪些特点)
- [new 和 make 是什么，差异在哪](#new-和-make-是什么差异在哪)
- [interface的实现原理](#interface的实现原理)
  - [runtime.eface](#runtimeeface)
  - [runtime.iface](#runtimeiface)
- [值接收者和指针接收者的区别](#值接收者和指针接收者的区别)
- [类型断言](#类型断言)
- [string的实现原理](#string的实现原理)
- [slice的实现原理](#slice的实现原理)
  - [slice的扩容](#slice的扩容)
- [map的现实原理](#map的现实原理)
  - [map的内部实现](#map的内部实现)
  - [map的扩容](#map的扩容)
  - [map并发安全吗](#map并发安全吗)
    - [三种线程安全的map实现方式](#三种线程安全的map实现方式)
- [Init()函数的作用](#init函数的作用)
- [进程、线程和协程的定义](#进程线程和协程的定义)
  - [线程和协程的差异](#线程和协程的差异)
  - [什么是goroutine](#什么是goroutine)
- [什么是channel](#什么是channel)
  - [导致Channel泄漏的原因](#导致channel泄漏的原因)
- [go gc的工作原理](#go-gc的工作原理)
  - [垃圾回收触发机制](#垃圾回收触发机制)
- [内存分配](#内存分配)
  - [堆内存分配的挑战](#堆内存分配的挑战)
  - [TC Malloc](#tc-malloc)
  - [Go 内存逃逸](#go-内存逃逸)
    ...
    - [产生逃逸的场景](#产生逃逸的场景)
- [内存整理](#内存整理)
- [导致panic的场景](#导致panic的场景)
  - [设计个消息队列](#设计个消息队列)
  - [实现超时取消](#实现超时取消)
- [什么时候用Var, 什么时候用 new/make申明变量](#什么时候用var-什么时候用-newmake申明变量)
...
![go slice](resources/go_slice.png)
...
Go语言中的Map是一种哈希表实现的无序键值对集合，底层使用了散列表。Map内部由一个桶数组和对应的哈希函数组成，每个桶包含一个指向键值对链表的指针。在Map中添加、查找和删除键值对时，会根据键的哈希值计算对应的桶的索引，并在该桶的链表中进行操作。为了减少哈希冲突，Go语言中的Map使用了链表式开放寻址法的解决方案。

### map的内部实现

![runtime hmap](resources/runtime_hmap.png)

![map hashcode](resources/map_hashcode.png)
...
![map resize](resources/map_resize.png)

### map并发安全吗

map不是并发安全的，map并没有对并发场景进行优化，并发读写是使用map时常见的错误, 如果多个goroutine同时访问一个map，就会出现并发问题，需要加锁保护。
...
- 协程占用资源小，每个协程的初始栈大小仅为2KB。远比java, C 的线程少：goroutine(2KB), 线程（8MB）
...
Go语言的垃圾回收器是一个三色标记清除的并发收集器. 三色标记 (三色标记：白色、灰色、黑色) 清除的工作原理是：

- 从根变量开始遍历所用的对象，引用的对象标记为“被引用”，没有被标记的进行回收。
...
Go内存整理是指将内存中的对象进行移动，从而使内存中的空闲空间连续起来，以便后续的内存分配。
...
- Nil Receivers
- Put a value into a channel that is already closed
- When a type assertion fails
- When a panic is explicitly called
